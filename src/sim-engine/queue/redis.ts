...
// (the orchestrator service persists + relays). Ported from run_manager.go.
// Key templates match the worker's (`config/rediscachekey.go:25-36`), with a configurable prefix
// (SIM_REDIS_PREFIX, default SIM_EVAL — matches the orchestrator service in the managed deployment; override for OSS); 3600s TTL.
//
...
// (managed-deployment prod) from one code path.
...
/** Stream + key TTL, matching the orchestrator service + the worker (3600s). */
...
// `prefix` defaults to the configured SIM_REDIS_PREFIX (SIM_EVAL in the managed deployment to match the orchestrator service on the
 ...
 * it never blocks on XREAD (the orchestrator service relays), so no dedicated blocking client is needed.
 */
 ...
 *  the orchestrator service's persist + relay parse AO's stream unchanged. Refreshes the stream TTL each append. */
...
/** Read the flow JSON the orchestrator service seeded for this run (worker GetFlowJSON, run_manager.go:236). */
