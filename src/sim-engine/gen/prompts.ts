...
export function writerSystemPrompt(): string {
  return [
    ...
    "",
    "ai_agent_v29 (AI Agent V2.9) nodes collect fields through a tool with automatic",
    "read-back confirmation: the agent reads each captured value back and confirms it",
    "without a separate prompt. A persona may volunteer a value (accepted and confirmed,",
    "not re-asked), decline a field (the flow continues), or correct an earlier value.",
    "Acceptance criteria for such a node must NOT demand validation it lacks (no",
    "future-date-only, no char-by-char read-back); they may reference the tool-backed",
    "collection and read-back confirmation behaviour.",
    "",
