...
export function writerSystemPrompt(): string {
  return [
    ...
    "  later).",
    "  For agent_node (Collect Input) nodes: same rule — the collection conversation runs",
    "  live, so OMIT the entry and let the persona/goal drive the answers. Include an entry",
    "  ONLY to pin a specific exit intent deterministically (outcome = the intent name),",
    "  with data_json carrying the variables that exit implies.",
