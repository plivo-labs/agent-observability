...
export async function runLlmJudge<T>(args: RunLlmJudgeArgs<T>): Promise<JudgeResult<T>> {
  ...
  try {
    const res = await completeJSON({
      ...
      // visible output near the parity size regardless. Default 25000 clears this
      // repo's own bar for the same model class (planner 24000; writer +
      // user-simulator uncapped) — and since max_output_tokens is a ceiling, not
      // a reservation, unused headroom costs nothing.
      maxTokens: args.maxTokens + (config.JUDGE_REASONING_HEADROOM_TOKENS ?? 25_000),
