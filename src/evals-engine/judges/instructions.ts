...
SEGMENT SCOPE — node_transcript is the ONLY evidence for this node's objective, procedure, policy and interaction quality. conversation_history is context for understanding references and prior state; any agent turn that appears there but not in node_transcript was spoken by ANOTHER node and is never this node's behaviour, achievement, or violation. next_node names the node the flow moved to after this segment (null when this segment ends the conversation): when next_node is set, the routing/handoff out of this node EXECUTED at the end of node_transcript — the exit is recorded silently and leaves no line — so never mark a handoff/transfer/route step as missed, never set achieved=false for not routing, and never count what the next node then said against this node's boundaries.
