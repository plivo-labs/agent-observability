...
9. META/INSTRUCTION LEAKAGE — a spoken Agent: line that verbalizes the agent's own internal control text IS hallucinated=true: reading out stage directions in the instruction register ("triggering Voicemail Detected silently", "no spoken content, as instructed", "[handing off — no goodbye of my own]", "I will leave a message for the automatically"), speaking intent/tool identifiers or function names to the caller, or reciting instruction text addressed to the agent itself. Source 3 grounds configured content that is MEANT to be spoken (scripted lines, persona, definitions, mandated scope statements); it does NOT ground meta-directives that describe how the agent should behave. CARVE-OUT — normal first-person call handling addressed TO the listener is NOT leakage and NOT a claim: "I'll go ahead and disconnect the call", "I'll transfer you now", "I'll leave a message", idle re-prompts and disconnect notices are ordinary dialogue (process narration per rule 4). Leakage requires the instruction register: bracketed/stage-direction text, tool or intent identifiers, directive phrasing about silence/triggering, or instruction text recited verbatim. Distinguish from rule 5: an unrendered {placeholder} is a substitution artifact (not hallucination); a fluently SPOKEN directive or tool/intent name is leakage. Quote the exact leaked words in technical_reason.
...
SCRIPTED CONTENT DELIVERY — how to apply the FCT to a mandated line that CARRIES INFORMATION. When a step scripts a specific line whose purpose is to convey enumerable information items to the listener (the agent's identity, the company/system name, the person being asked for, locations, dates, amounts — e.g. a line a call screener or gatekeeper must relay to someone else), the step's OBJECTIVE includes conveying those items, not merely speaking a sentence in the right slot. Paraphrase that preserves every substantive item = objective achieved → FCT applies, deviation minor. Omitting substantive items the script exists to convey = the objective was only PARTIALLY achieved → that is a genuinely missed step, not a wording deviation; list it in missed_steps. Severity still follows the gating question: "critical" when the omissions defeat the step's purpose (the listener could not act/relay/identify from what was actually said — e.g. a screener told only "I'm calling about your move" with the agent's name, the system name, the lead's name, and the move details all dropped) and the step exists to verify identity, obtain consent, deliver a disclosure, or route/relay correctly; otherwise "minor". This narrows ONLY the information-carrying-script case; pure style/wording/order deviations remain minor under the FCT.
...
2. If there is no user input, or the chosen intent is null/empty/"(none)" → the framework did not RECORD one; that alone is NOT a wrong identification. Judge from the conversation and Tool_Call/handoff evidence:
   a. intent_wrongly_identified=true ONLY when the evidence shows an intent that contradicts the user's actual request.
   b. PLATFORM FACT — an intent fires ONLY via a line labelled Tool_Call:/Agent_Handoff:. An agent turn whose TEXT narrates or mentions triggering ("[Triggering Voicemail Detected silently]", "handing off now", "functions.handoff_x") did NOT fire anything — narration is agent turn content, never a fired tool. intent_not_found=true when the user expressed a clear need absent from the available list, OR when BOTH hold: (i) a caller-side turn satisfies an available intent's stated trigger condition in its own terms; for a declared voicemail-type intent these ARE unambiguous triggers, never merely "suggestive": "you've reached <name>" + a leave-a-message request, "the person you're trying to reach is not available", "your call has been forwarded to voicemail / an automated voice messaging system", "leave a message after the tone/beep", a mailbox announcement. An explicit "do not call me again" is the trigger for a declared opt-out intent. AND (ii) the agent produced ANY turn content during the call — live-script speech or a narrated trigger — yet no intent tool ever fired by call end. A mandated exit the agent talked or narrated past is a real miss, even when the trigger was the last thing heard: do not excuse it because the call ended right after, because the narration "shows intent", or because firing was impractical — if the trigger in (i) matched and agent turns exist, fire.
   c. SUPPRESSION — narrow, apply ONLY these three: (1) the transcript contains NO agent turns at all — exit handling then happened outside the transcript and the absence of a recorded intent is not evidence of a miss; (2) the caller-side turn does not actually meet the intent's stated condition — e.g. a screener asking who is calling or to record a name, hold music, or an ambiguous human "hello" (a screener line counts as a voicemail trigger only when it says the person is unavailable or offers to take a message AND the node's own rules say to treat that as voicemail); (3) a DIFFERENT declared intent was correctly fired for the same situation. Genuine ambiguity → both false; but the listed carrier/voicemail phrasings in (i) are not ambiguous.
   Stop.
...
4a. Consent gate: when the chosen intent's own instructions require the caller to explicitly AGREE, CONFIRM, or REQUEST something (transfer approval, callback confirmation, opt-in), check the caller's turns after the qualifying question was asked — anchored on the reply immediately before the intent fired. If the caller's answer was a decline/deferral ("no", "not now", "I'll call you back", "I found someone else") and no later turn reversed it, the consent condition was NOT met → intent_wrongly_identified=true. This holds even when the agent's SPOKEN message was appropriate (e.g. it spoke a follow-up question or acknowledged the refusal) — judge the fired intent against the caller's actual answer, not against what the agent said.
