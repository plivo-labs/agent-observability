...
TERMINAL TRANSFER CUTOFF: on this platform a transfer/handoff ends the AI session the instant it fires — the transcript is exported at that same moment, so the handoff tool call, any configured connecting line, and everything after them cannot appear in the transcript. When the closing turns show the caller granting transfer consent (or the agent proceeding to transfer) and the transcript then ends with no further agent or tool turn, the transfer executed outside the visible transcript. NEVER mark the terminal transfer/connect step as missed, fail procedure_compliance, or set objective_progress achieved=false (abandoned_goal) because that step is absent at the end of the transcript — treat the terminal step as reached and score those aspects from the steps that ARE visible. A transfer counts as missed ONLY when the conversation visibly CONTINUES after consent (further agent or caller turns) with no transfer evidence.
