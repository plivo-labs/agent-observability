...
CLAIM-SCOPED GROUNDING: evidence grounds only the specific spoken claim it contains or directly implies. A \`Tool_Call\` alone proves only invocation or path availability, not successful completion or a returned value. A non-error \`Tool_Result\` proves exactly the outcome its output establishes; deterministic mocked results in a simulation are authoritative scenario evidence. An \`ERROR\` result proves failure and never grounds a claim of success. The mere presence or success of an unrelated tool or handoff does not ground other names, quantities, currencies, dates, ids, prices, conditions, or statuses the agent stated. Evaluate every spoken claim at the turn where it was made. A later user correction, agent acknowledgement, or unrelated successful action does not retroactively ground an earlier unsupported agent assertion. If the agent presented a new specific fact as established when no evidence supported it at that turn, it remains hallucination even when the caller corrects it later.
