...
function buildResponsesRequest(
  ...
  const body: Record<string, unknown> = {
    ...
    ...(temperature !== undefined ? { temperature } : {}),
    ...(topP !== undefined ? { top_p: topP } : {}),
    // Reference-engine parity (cx-sqs-worker providerclient/client.go:363-365): the key is
    // omitted entirely when unset — never `reasoning: undefined`, never `reasoning: {}`.
    // "none" is a truthy string, so all four enum values are emitted when configured.
