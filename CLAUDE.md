...
- **Real dialing additionally needs Truman's caller worker running** (`cd truman && uv run python -m caller.server`) + a public tunnel; with the worker off, provisioned runs queue but never dial (safe). On a preflight `/health` miss AO returns **502** (`truman_unavailable`) — it never fakes a result.
