...
- Assertion layer (the orchestrator loop + evals): agent-runner owns the walk; AO decides pass/fail.
  ...
  - **Route assertion** (whole-run, no LLM): a scenario's `eval_metadata.expected_route_outcome` (`source_node_id` + `expected_intent_name` + `target_node_id`, all required) passes iff some turn fired the expected intent at the source and the walk landed on the target. A failure on an otherwise-normal completion becomes `route_mismatch` (never masks a walker abort); every turn's `transitions` ride the `turn_completed` payload.
  ...
  - **Stop reasons**: `caller_goal_met`, `caller_hung_up`, `route_mismatch` join the vocabulary and are NOT abort reasons — their rows keep `error = null` so they count toward passed/failed.
  - **Pass/fail** (`extractGoalPassed`, mirror the orchestrator-service port): error/eval_error → null; `route_mismatch` → false; `criteria_evaluation.passed` (authoritative over flow goals); else the ANY-goal rule; nothing to judge → null. `target_achieved` only ends the call — it is not the verdict.
