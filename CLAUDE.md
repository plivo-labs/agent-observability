...
In-progress/just-finished runs survive a browser refresh via `frontend/src/components/simulate/run-persistence.ts` (no backend change). The only server-recoverable handle is the Truman **`suiteId`** (re-fetched with `getSuiteStatus` → `GET /api/calls/batch/:suiteId`); both pages persist a small localStorage blob (`ao.live.run` / `ao.sim.run`, version-stamped) on start and **rehydrate on mount** — set `batch`/`voiceSuite` + `phase` and the existing poll effects + `useLiveCall` resume on their own. The handle is cleared on terminal/`New suite`/`New simulation` (so a finished suite doesn't auto-reopen) and on a 404 re-fetch. The **Live elapsed timer** is seeded from a persisted `startedAt` (the `clock` counter otherwise restarts at 0). A fresh Library→Scenario navigation (`location.state.scenario`) wins over a saved blob.

#### Text sim is now a SERVER-side JOB (resumable across refresh AND in-app nav)
The text sim used to run **inside** the React page (`runSimulationStream`/`runSimulation` driven by `simulate-page.tsx`), so unmounting (in-app nav) or refreshing aborted it — there was no server handle, so the page could only offer **Re-run** ("interrupted by a refresh and couldn't be resumed"). Fixed by giving the run a server-recoverable handle:
- **Backend** (`src/simulation/jobs.ts` — module-level `Map<jobId, JobState>`, cap 50 / 30-min TTL pruned on create): `POST /api/simulations/jobs` validates with `simRequestSchema`, mints a `jobId`, kicks off `runSimulation(parsed, undefined, onEvent)` in the **background** (NOT awaited), and returns `{ jobId }` immediately. `onEvent` folds engine events into the job (`start`→seed `cases`; `turn`→push `cases[i].turns`; `case_done`→status/score). On resolve → `persistSimRun(result)`, `status:'done'`, `result` (with `evalRunId` merged on), `runId`. On reject → `status:'error'`. `GET /api/simulations/jobs/:id` returns the `JobState` (404 when unknown/expired/cleared). The legacy `POST /api/simulations` + `/stream` stay for back-compat. **These are new routes → restart the backend ONLY via `python3 /tmp/ao-dev-restart-azure.py`** (a plain restart drops Azure/LLM env → sim falls back to `engine:"demo"`; the script preserves DATABASE_URL/TRUMAN/Azure + AUTO_MIGRATE).
- **Frontend**: `sim-data.ts` adds `startSimulationJob(req)` / `getSimulationJob(jobId)` + exports `JobState`. `run-persistence.ts` `SimRun` gained `jobId` (the resume handle; `lastResult` kept as an instant-restore snapshot). `simulate-page.tsx` controller: **Run** → `startSimulationJob` → persist `{config, jobId}` → phase `running` → **poll** `getSimulationJob` (~1s) driving `LiveCase[]` from `job.cases`; `done` → set `result` + report, `error` → show error. **On mount** (refresh OR returning to the tab) it reads the persisted `jobId` and re-fetches: `running` → resume polling + Running UI; `done` → report; **404 → only THEN** clear + offer Re-run. So in-app nav (Monitor → Simulate) auto-resumes too. The old unconditional "interrupted by a refresh" banner now appears **only** on a true 404 (expired / backend restart cleared the map); copy updated accordingly. Cancel/`New simulation`/`rerun` clear the jobId + `clearSimRun`. The old in-page stream is dropped for the primary path.
- **Verified** (engine stayed `llm` — Azure intact): `POST /api/simulations/jobs` → `{jobId}` immediately; `GET .../jobs/:id` → `running` (cases seeded, turns streaming) → `done` with `result` (`engine:"llm"`, `overall`, `runId`/`evalRunId`); unknown id → 404. CDP-drove :5174: (a) refresh mid-run → Running UI restored, no banner; (b) Monitor→Simulate mid-run → Running UI restored; (c) finished+reload → full report, no banner; (d) bogus jobId → interrupted+Re-run banner shows.
