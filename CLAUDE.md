...
- `src/sim-engine/` — the simulation engine: `gen/` (planner → deterministic allocator → writer LLM pipeline, streamed over SSE), `run-engine/` (the user-simulator loop + stress + the agent-runner sim `livekit-client.ts`), `queue/` (SQS consumer + the Redis `:RESULTS` stream and Lua completion gate), `routes.ts` (`/api/simulation/*`), `db.ts` (the `ao_sim_*` tables). SER-6447: the flow walk (entry from Start, edge resolution, branch evaluation, non-AI node mocking, stop reasons) moved to agent-runner — AO sends `flow` + `world_state` per turn and consumes the walk result (`turn_node_uuid` speaker / `node_uuid` landing / `transitions` / `stop_reason` + `stop_detail`); it no longer parses the graph. `orchestrator.ts` drives stateless `turn()` for ai_agent_v2 and the server-held flow-session endpoints for task units, keying the transcript on the speaking node and sending an empty-user greeting turn when the walk lands on a new conversational node. Generation dry-runs the flow through `POST /v1/simulation/flow/inventory` once and refuses an unsimulatable flow (400 `flow_not_simulatable`).
