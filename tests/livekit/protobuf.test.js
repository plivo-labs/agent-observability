...
import { decodeMetricsRecordingHeader, decodeOtlpLogsRequest, } from "../../src/livekit/protobuf.js";
...
function timestamp(seconds, nanos = 0) {
    ...
    if (nanos > 0)
        parts.push(varintField(2, nanos));
    return Buffer.concat(parts);
}
function stringMapEntry(key, value) {
...
describe("decodeOtlpLogsRequest", () => {
    function jsonLogsRequest(records) {
        return {
            resourceLogs: [
                {
                    scopeLogs: [
    ...
    function jsonValue(value) {
        ...
        if (value && typeof value === "object") {
            const kvList = Object.entries(value).map(([k, v]) => ({
                key: k,
                value: jsonValue(v),
            }));
    ...
    test("decompresses gzipped JSON bodies (content-encoding: gzip)", () => {
        ...
        const logs = decodeOtlpLogsRequest(new Uint8Array(compressed), "gzip", "application/json");
        expect(logs).toHaveLength(1);
        expect(logs[0].body).toBe("session report");
