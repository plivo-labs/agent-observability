...
// ── Mocks ──────────────────────────────────────────────────────────────────
const mockInsertEvalRun = mock((_payload) => Promise.resolve());
const mockCountEvalRuns = mock((_opts) => Promise.resolve(0));
const mockListEvalRuns = mock((_opts) => Promise.resolve([]));
const mockGetEvalRun = mock((_runId) => Promise.resolve(null));
const mockListEvalCases = mock((_runId) => Promise.resolve([]));
const mockGetEvalCase = mock((_runId, _caseId) => Promise.resolve(null));
const mockDeleteEvalRuns = mock((_runIds) => Promise.resolve(0));
const mockSql = mock((..._args) => Promise.resolve([]));
...
describe("POST /observability/evals/v0", () => {
    ...
    test("rejects request without auth", async () => {
        const res = await server.fetch(makeRequest("/observability/evals/v0", {
            method: "POST",
    ...
    test("translates legacy payload (framework=vitest, sdk=livekit-agents)", async () => {
        ...
        delete legacy.run.framework;
        delete legacy.run.framework_version;
        delete legacy.run.testing_framework;
        delete legacy.run.testing_framework_version;
        ...
        expect(called.run.sdk).toBeUndefined();
        expect(called.run.sdk_version).toBeUndefined();
    });
    test("returns 500 when insert fails", async () => {
        mockInsertEvalRun.mockImplementationOnce(() => Promise.reject(new Error("db down")));
        const res = await server.fetch(makeRequest("/observability/evals/v0", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
...
describe("GET /api/evals", () => {
    ...
    test("returns paginated list with defaults", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(2);
        ...
        const res = await server.fetch(makeRequest("/api/evals", { headers: { Authorization: basicAuthHeader() } }));
    ...
    test("clamps limit to 50", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(0);
        mockListEvalRuns.mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/evals?limit=999", {
    ...
    test("forwards filters to db opts", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(0);
        mockListEvalRuns.mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/evals?agent_id=support-bot&framework=livekit&testing_framework=pytest&account_id=acct-1", { headers: { Authorization: basicAuthHeader() } }));
        expect(res.status).toBe(200);
    ...
    test("accepts multi-value framework filter (comma-separated)", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(0);
        mockListEvalRuns.mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/evals?framework=livekit,pipecat", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const opts = mockListEvalRuns.mock.calls[0][0];
    ...
    test("accepts multi-value testing_framework filter (comma-separated)", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(0);
        mockListEvalRuns.mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/evals?testing_framework=pytest,vitest", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const opts = mockListEvalRuns.mock.calls[0][0];
    ...
    test("pagination links preserve filters", async () => {
        mockCountEvalRuns.mockResolvedValueOnce(30);
        mockListEvalRuns.mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/evals?limit=10&offset=10&agent_id=support-bot", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
...
describe("GET /api/evals/:run_id", () => {
    ...
    test("returns run + cases", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/evals/r1", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
    ...
    test("returns 404 when run not found", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/evals/nope", {
            headers: { Authorization: basicAuthHeader() },
        }));
...
describe("GET /api/evals/:run_id/cases/:case_id", () => {
    ...
    test("returns case", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/evals/r1/cases/c1", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
    ...
    test("returns 404 when case not found", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/evals/r1/cases/nope", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(404);
        const body = await res.json();
