...
function liveKitBearerHeader(overrides, secret = LIVEKIT_API_SECRET) {
...
function fieldTag(field, wire) {
    return varint((field << 3) | wire);
}
function lengthDelimited(field, body) {
...
function buildRecordingHeader(roomId, startSeconds) {
...
function anyValue(value) {
    ...
    if (value && typeof value === "object") {
        const body = Buffer.concat(Object.entries(value).map(([key, item]) => lengthDelimited(1, keyValue(key, item))));
...
function buildJsonLogsRequest(records) {
    return {
        resourceLogs: [
            {
                resource: {
                ...
                scopeLogs: [
                    {
                        ...
                        logRecords: records.map((record, index) => ({
                            timeUnixNano: String((BigInt(Date.now()) + BigInt(index)) * 1000000n),
...
describe("POST /observability/recordings/v0", () => {
    ...
    test("rejects request without auth header", async () => {
        ...
        const res = await server.fetch(makeRequest("/observability/recordings/v0", {
            method: "POST",
            body: form,
        }));
    ...
    test("accepts valid LiveKit Bearer auth", async () => {
        ...
        const call = mockInsertSession.mock.calls[0][0];
...
describe("POST /observability/logs/otlp/v0", () => {
    ...
    test("accepts OTLP tag, session report, chat item, and evaluation records", async () => {
        ...
        const res = await server.fetch(makeRequest("/observability/logs/otlp/v0", {
            method: "POST",
...
describe("GET /api/sessions", () => {
    ...
    test("passes account_id filter as a case-insensitive LIKE predicate", async () => {
        mockSql
            .mockResolvedValueOnce([{ total: 0 }])
            .mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/sessions?account_id=Acct-XYZ", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        // Count + rows = 2 SQL calls; both must include the WHERE clause and the param.
        expect(mockSql).toHaveBeenCalledTimes(2);
        const [countQuery, countParams] = mockSql.mock.calls[0];
        expect(countQuery).toContain("WHERE LOWER(account_id) LIKE");
        expect(countParams).toEqual(["%acct-xyz%"]);
        const [rowsQuery, rowsParams] = mockSql.mock.calls[1];
    ...
    test("escapes LIKE metacharacters in account_id input", async () => {
        mockSql
            .mockResolvedValueOnce([{ total: 0 }])
            .mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/sessions?account_id=" + encodeURIComponent("50% off_lab"), {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const [, countParams] = mockSql.mock.calls[0];
    ...
    test("passes started_from/started_to filters as timestamp predicates", async () => {
        ...
        const res = await server.fetch(makeRequest(`/api/sessions?started_from=${encodeURIComponent(from)}&started_to=${encodeURIComponent(to)}`, {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        expect(mockSql).toHaveBeenCalledTimes(2);
    ...
    test("combines account_id + date range into a single WHERE clause", async () => {
        ...
        const from = "2026-04-01T00:00:00.000Z";
        const to = "2026-04-30T23:59:59.999Z";
        const res = await server.fetch(makeRequest(`/api/sessions?account_id=acct-1&started_from=${encodeURIComponent(from)}&started_to=${encodeURIComponent(to)}`, { headers: { Authorization: basicAuthHeader() } }));
        expect(res.status).toBe(200);
        const [countQuery, countParams] = mockSql.mock.calls[0];
    ...
    test("pagination links preserve active filters", async () => {
        mockSql
            .mockResolvedValueOnce([{ total: 30 }])
            .mockResolvedValueOnce([]);
        const res = await server.fetch(makeRequest("/api/sessions?limit=10&offset=10&account_id=acct-1&started_from=2026-04-01", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
...
describe("GET /api/sessions/:id", () => {
    ...
    test("returns native session evaluation data", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/sessions/sess-eval", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
    ...
    test("sorts session events by created_at", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/sessions/sess-events", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
    ...
    test("normalizes legacy stringified raw_report arrays for events and options", async () => {
        ...
        const res = await server.fetch(makeRequest("/api/sessions/sess-legacy-raw", {
            headers: { Authorization: basicAuthHeader() },
        }));
        expect(res.status).toBe(200);
        const body = await res.json();
