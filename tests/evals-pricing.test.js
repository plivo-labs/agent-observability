...
import { priceFor, normalizeProvider, normalizeModel, reloadPrices, __setPricesForTesting, __resetPricingForTesting, } from "../src/evals/pricing.js";
...
describe("normalizeModel", () => {
    test("lowercases and strips dated suffixes", () => {
        ...
        expect(normalizeModel("GPT-4o-2024-08-06")).toBe("gpt-4o");
        expect(normalizeModel("claude-sonnet-4-5-20241022")).toBe("claude-sonnet-4-5");
    });
...
describe("reloadPrices (models.dev fetch)", () => {
    ...
    test("empty parse result preserves the existing map", async () => {
        ...
        globalThis.fetch = mock(async () => new Response(JSON.stringify({ openai: { models: {} } }), { status: 200 }));
        await reloadPrices();
        expect(priceFor("openai", "gpt-4o-mini")?.input).toBe(0.15);
    });
});
