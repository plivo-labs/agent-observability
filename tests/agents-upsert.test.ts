...
const sqlCalls: Array<{ values: any[] }> = [];

// Tagged template mock that records (strings, ...values) on each call.
const mockSqlTag = mock((_strings: TemplateStringsArray, ...values: any[]) => {
...
describe("upsertAgentTx", () => {
  ...
  test("routes through the supplied transaction handle, not the shared sql tag", async () => {
    const txCalls: any[][] = [];
    const tx: any = (_strings: TemplateStringsArray, ...values: any[]) => {
      txCalls.push(values);
      return Promise.resolve();
    };
  ...
  test("same null pass-through as upsertAgent", async () => {
    const txCalls: any[][] = [];
    const tx: any = (_s: TemplateStringsArray, ...values: any[]) => {
      txCalls.push(values);
      return Promise.resolve();
    };
  ...
  test("throws when agentId is missing", async () => {
    const tx: any = mock(() => Promise.resolve());
    await expect(
      upsertAgentTx(tx, { agentId: "" }),
    ).rejects.toThrow("agentId is required");
