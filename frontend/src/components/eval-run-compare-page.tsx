...
export function EvalRunComparePage({
  ...
  const DiffRow = ({ d }: { d: DiffItem }) => {
    ...
    return (
      <div className="eval-diff-row" onClick={() => c && onOpenRun?.(c.run_id)}>
        ...
        <span>{d.a ? <StatusPill status={d.a.status} /> : <span className="text-muted-foreground">—</span>}</span>
        <span className="inline-flex items-center gap-1.5 font-mono text-xs-500">
          <span className="text-muted-foreground">→</span>
          {d.b ? <StatusPill status={d.b.status} /> : <span className="text-muted-foreground">—</span>}
        </span>
        <span className="mono-pair font-mono text-xs-500">
          {d.a?.ttft_p50_ms != null && d.b?.ttft_p50_ms != null ? (
            <>
              <span className="text-muted-foreground">{formatMs(d.a.ttft_p50_ms)}</span>
              <span className="text-muted-foreground">→</span>
  ...
  if (loadingA || loadingB) {
    return (
      <div className="w-full p-6 flex flex-col gap-4">
        <AgentScopeHeader agentId={agentId} trail={compareTrail} />
        <div className="p-10 text-center text-muted-foreground font-mono text-xs-500">
          Loading runs…
        </div>
      </div>
    )
  ...
  return (
    <div className="w-full p-6 flex flex-col gap-4 min-w-0">
      ...
      <div className="text-s-400 text-muted-foreground -mt-2">
      ...
      {unchanged.length > 0 && (
        <details className="mt-3">
          <summary className="cursor-pointer p-2 text-xs-500 text-muted-foreground">
