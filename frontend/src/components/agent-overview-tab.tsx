...
// Small KPI card — used along the top row of the dashboard. The optional
// tone hint colors the value to communicate health at a glance.
function Kpi({
  label,
  value,
  hint,
  ...
  return (
    <Card>
      <CardContent className="pt-6">
        ...
        <div className={'mt-1 text-2xl ' + toneClass}>{value}</div>
        {hint && (
...
function passRateTone(rate: number | null | undefined): 'good' | 'warn' | 'bad' | 'neutral' {
...
export const AgentOverviewTab = ({ agentId, range }: AgentOverviewTabProps) => {
  ...
  const ciTone = passRateTone(stats.ci_pass_rate)
  const llmTone = passRateTone(stats.llm_pass_rate)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi
          ...
          hint={`avg ${stats.avg_turn_count?.toFixed(1) ?? '—'} turns`}
        />
        <Kpi
          label="p95 perceived latency"
          value={formatMs(stats.p95_user_perceived_ms)}
          hint={`p50 ${formatMs(stats.p50_user_perceived_ms)} · p99 ${formatMs(
        ...
        <Kpi
          ...
          tone={llmTone}
          hint="conversation-level judges"
        />
        <Kpi
          label="Simulation pass rate"
          value={formatPct(stats.ci_pass_rate)}
          tone={ciTone}
          hint="scenarios run against the agent"
        />
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader className="pb-1">
                <CardTitle className="text-sm font-medium">
                  p95 perceived latency over time
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-2">
                <ResponsiveContainer width="100%" height={220}>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <Card>
              <CardHeader className="pb-1">
                <CardTitle className="text-sm font-medium">
                  Avg session duration over time
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-2">
                <ResponsiveContainer width="100%" height={220}>
              ...
              <CardContent className="pt-2">
                {stats.provider_breakdown.length === 0 ? (
                  <div className="flex h-[220px] items-center justify-center text-sm text-muted-foreground">
                    No provider metadata in this window
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
