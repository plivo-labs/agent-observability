    ...
    #walk(task, parentName) {
        ...
        if (task.type === "test") {
            this.cases.push({
                id: task.id,
                name: fullName,
                outcome: (task.result?.state ?? "unknown"),
...
// ── HTTP server ─────────────────────────────────────────────────────────────
async function handleRunVitest(body) {
    if (!process.env.OPENAI_API_KEY) {
        return new Response(JSON.stringify({ error: "OPENAI_API_KEY is not set" }), { status: 400, headers: { "content-type": "application/json" } });
    }
    const n = body.n ?? 10;
    ...
    const vitest = await startVitest("test", [testPath], { watch: false, run: true, reporters: [reporter] });
    if (!vitest) {
        return new Response(JSON.stringify({ error: "vitest failed to start" }), { status: 500, headers: { "content-type": "application/json" } });
    }
...
async function handleRunScenarios(body) {
    if (!process.env.OPENAI_API_KEY) {
        return new Response(JSON.stringify({ error: "OPENAI_API_KEY is not set" }), { status: 400, headers: { "content-type": "application/json" } });
    }
    const n = body.n ?? 10;
...
Bun.serve({
    ...
    async fetch(req) {
        ...
        if (url.pathname === "/run/vitest")
            return handleRunVitest(body);
        if (url.pathname === "/run/scenarios")
            return handleRunScenarios(body);
