...
const { tool } = llm;
import { z } from "zod";
import { generateScenarios, runScenarios, requireOpenAIKey, } from "./scenarioRunner.js";
// ── Agent under test ────────────────────────────────────────────────────────
const MENU = {
...
export class PizzaShopAgent extends Agent {
    constructor() {
        super({
            instructions: "You are the voice order-taker at Tony's Pizza. You can look up the " +
            ...
            tools: {
                get_menu: tool({
                    ...
                    execute: async () => "Menu:\n" +
                        Object.entries(MENU)
                            .map(([n, i]) => `- ${n}: $${(i.priceCents / 100).toFixed(2)} — ${i.desc}`)
                            .join("\n"),
                }),
                place_order: tool({
                    ...
                    execute: async ({ items, delivery_address }) => {
                        ...
                        const total = items.reduce((s, i) => s + MENU[i.toLowerCase()].priceCents, 0);
                        ORDERS[orderId] = {
                            status: "received",
                            items,
...
export const SPEC = {
    ...
    instructions: "Take orders for menu items only. Confirm totals and address before " +
...
const SCENARIOS = await generateScenarios(SPEC, N);
...
describe("LLM-generated PizzaShopAgent scenarios", () => {
    ...
    it.each(SCENARIOS.map((s) => [s.name, s]))("scenario: %s", async (_name, scenario) => {
